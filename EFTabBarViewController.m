...
- (IBAction)tabBarItemTapped:(id)sender {
    ...
    if([self.delegate respondsToSelector:@selector(EFTabBarViewController:selectedIndex:)]){
        EFTabBarItemButton *barItemButton = (EFTabBarItemButton*)sender;
        [self.delegate EFTabBarViewController:self selectedIndex:barItemButton.tag];
    }
}
...
- (void)setSelectedIndex:(int)selectedIndex{
    ...
    int i = 0;
    for (EFTabBarItemButton *button in self.itemButtons) {
...
- (id)initWithBackgroundImage:(UIImage*)image viewControllers:(id)firstObj, ... {
    ...
    if (self) {
        ...
        NSMutableArray *viewControllers = [[[NSMutableArray alloc] initWithObjects:firstObj,nil] autorelease];
        id obj;
        ...
        self.tabBarView = [[[AMBlurView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - kTabBarHeight+4, self.view.frame.size.width, kTabBarHeight-4)] autorelease];
        
        self.tabBarBackgroundImageView = [[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.tabBarView.frame.size.width, kTabBarHeight-4)] autorelease];
        ...
        for (id obj in _viewControllers) {
            EFTabBarItemButton *button = [[(EFTabBarItemButton*)[EFTabBarItemButton alloc] init] autorelease];
            ...
            [_itemButtons addObject:(EFTabBarItemButton*)button];
            i++;
        }
...
- (void)hideTabBarWithAnimation:(BOOL)isAnimated{
    if (isAnimated) {
        ...
        if(IS_WIDESCREEN){
            ...
            [self.tabBarView setFrame:CGRectMake(0, 460+20, self.tabBarView.frame.size.width, self.tabBarView.frame.size.height)];
        }
